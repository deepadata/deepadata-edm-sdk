 ...
 */
export function validateProfileConformance(artifact: unknown): ProfileConformanceResult {
  const errors: ProfileConformanceError[] = [];
 ...
 */
function validateGovernanceNested(
  ...
  if (governance.retention_policy && typeof governance.retention_policy === "object") {
    const rp = governance.retention_policy as Record<string, unknown>;
  ...
  if (governance.subject_rights && typeof governance.subject_rights === "object") {
    const sr = governance.subject_rights as Record<string, unknown>;
 ...
 */
function detectProfile(artifact: unknown): EdmProfile {
  if (!artifact || typeof artifact !== "object") {
    return "full";
  }
  const obj = artifact as Record<string, unknown>;
  const meta = obj.meta as Record<string, unknown> | undefined;
  const profile = meta?.profile as string | undefined;
 ...
 */
export function validateEDMWithProfile(artifact: unknown): ValidationResult & { profileResult: ProfileConformanceResult } {
