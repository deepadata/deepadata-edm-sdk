...
const STRICT_ENUMS: Record<string, Record<string, readonly string[]>> = {
  constellation: {
...
const ARRAY_CAPS: Record<string, Record<string, number>> = {
  constellation: { emotion_subtone: 4 },
  gravity: { resilience_markers: 3 },
};

/** Numeric fields clamped to [0, 1] */
const UNIT_INTERVAL_FIELDS: Record<string, readonly string[]> = {
...
export function sanitizeLlmOutput(parsed: unknown): SanitationNote[] {
  const notes: SanitationNote[] = [];
  if (typeof parsed !== "object" || parsed === null) return notes;
  ...
  for (const [domain, fields] of Object.entries(STRICT_ENUMS)) {
    const d = root[domain] as Record<string, unknown> | undefined;
...
export function formatSanitationNotes(notes: SanitationNote[]): string | null {
