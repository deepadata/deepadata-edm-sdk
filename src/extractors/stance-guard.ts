...
function buildClassifierUserContent(input: StanceClassifierInput): string {
  return (
...
export async function classifyStanceAnthropic(
  client: Anthropic,
  model: string,
  input: StanceClassifierInput
): Promise<ExperientialStance | null> {
...
const CONSERVATISM: Record<ExperientialStance, number> = {
  lived: 0,
  witnessed: 1,
  hypothetical: 2,
...
export function resolveStance(
  claimed: ExperientialStance | null,
  classified: ExperientialStance | null
): ExperientialStance | null {
  if (classified === null) return claimed;
  if (claimed === null) return classified;
